...
<div class="row">
	...
	{ 
		$id_noticia = (int) mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['result']); // Recebemos o ID das notícias por meio de GET
		$query_NoticiaCompleta = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM cms_news WHERE id = '".$id_noticia."'"); // Nós executamos a consulta
		$columna_MostrarNoticia = mysqli_fetch_assoc($query_NoticiaCompleta);
		$query_Results = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM cms_news_form WHERE news_id = '".$id_noticia."' ORDER BY id DESC LIMIT 100"); // Nós executamos a consulta
		$columna_Resultados = mysqli_fetch_assoc($query_Results);		
