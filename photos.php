...
{ 
    if(!empty($_GET['url']))
    { 
        $id_photo = (int) mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['url']); 
        $query_photos = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM camera_web WHERE id = '".$id_photo."' LIMIT 1");
        if(mysqli_num_rows($query_photos) > 0)
        { 
            $columna = mysqli_fetch_assoc($query_photos);
			$user_n = mysqli_fetch_assoc(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM users WHERE id = '". $columna['user_id'] ."'"));
			$room_n = mysqli_fetch_assoc(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM rooms WHERE id = '". $columna['room_id'] ."'"));
			$roomowner_n = mysqli_fetch_assoc(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM users WHERE id = '". $room_n['owner_id'] ."'"));
			$photo = '' . $columna['id'] . '';
...
<?php
$photos = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM camera_web WHERE user_id = '".$user_n['id']."' ORDER BY id DESC");
