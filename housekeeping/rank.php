...
<?php 
$getrank = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM permissions ORDER BY id ASC");
...
$Geturank = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM users WHERE username = '". $Miembro ."'");
...
elseif($Fallo == false)
{
	mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE users SET rank = '". $Rango ."' WHERE username = '". $Miembro ."' LIMIT 1");
    mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO stafflogs (action, message, note, userid, timestamp) VALUES ('Housekeeping', 'Dió rango a $Miembro', '". $myrow['rank'] ."', '". $myrow['id'] ."', '". $date_full ."')");
    echo '<div class="col s12 m12">
...
<?php $ourstaffs = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM users WHERE rank > 4 ORDER BY rank DESC");
while($ourstaff = mysqli_fetch_array($ourstaffs)){
$rank = mysqli_fetch_assoc($rank = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM permissions WHERE id = '".$ourstaff['rank']."'"));
?>	
<tr role="row">
