...
{ 
    if(!empty($_GET['url']))
    { 
        $id_noticia = (int) mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['url']); 
        $query_noticias = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM cms_news WHERE id = '".$id_noticia."' LIMIT 1");
        if(mysqli_num_rows($query_noticias) > 0)
        { 
            $columna = mysqli_fetch_assoc($query_noticias);
			$user_n = mysqli_fetch_assoc(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT id,username,look,user_style FROM users WHERE username = '". $columna['author'] ."'"));
			$noticia = '' . $columna['id'] . '';
...
{
	$urlresultado = mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST['urlresultado']);

    mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO cms_news_form SET user_id = '". $myrow['id'] ."', news_id = '". $noticia8 ."', urlresultado = '". $urlresultado ."', date = '". time() . "'");
...
<?php $news = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM cms_news WHERE author = '".$noticia6."' ORDER BY id DESC LIMIT 6");
...
?>
	...
	<div class="card-body">
				<h5 class="card-title mb-4"><a href="/news/<?php echo $new['id']; ?>" data-toggle="tooltip" title="" data-original-title="<?php echo mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $new['title']); ?>"><?php echo mysqli_real_escape_string($GLOBALS["___mysqli_ston"], mb_strimwidth($new['title'], 0, 26, "...")); ?></a></h5>
		<div class="card-text">
			<div class="avatar pixel sm mr-2">
