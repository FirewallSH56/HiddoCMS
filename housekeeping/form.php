...
$get = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM cms_news WHERE id = '".$idd."' ");

if(mysqli_num_rows($get) == 1)
{
    $nws = mysqli_fetch_object($get);
}else
...
?>
	  ...
	  <div class="form-group">
	  Foram enviados <b><?php echo mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM cms_news_form WHERE news_id = '".mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $nws->id)."'")) ?></b> resultados.
	  </div>
	  <hr>

<?php $forms = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM cms_news_form WHERE news_id = '".mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $nws->id)."' ORDER BY id DESC");
while($form = mysqli_fetch_array($forms)){
$user = mysqli_fetch_assoc($user = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM users WHERE id = '".$form['user_id']."'"));
?>
					<div class="form-group">
						...
						<span class="form-text text-muted">Dados enviados dia <?php echo date('d/m', $form['date']); ?> de <?php echo date('Y', $form['date']); ?> na promoção: <?php echo mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $nws->title); ?></span>
                    </div><hr>
<?php } ?>
